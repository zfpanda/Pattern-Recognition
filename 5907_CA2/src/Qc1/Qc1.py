...
weights = ((np.linalg.inv(np.dot(x_train.T,x_train))).dot(x_train.T)).dot(y_train_10)

# predict test set output
pre = np.dot(x_test,weights)
...
print('Total errors number is',error)

# Accuracy of classification  0.8331999
print('Accuracy of classification ',round(1-error/len(index),4))

#Confusion matrix
confusion_mat=confusion_matrix(y_test,index)
def plot_confusion_matrix(confusion_mat):     
    ...
    plt.savefig('Confusion Matrix by MSE Using Indicator Matrix Primal.png')
    plt.show()
...
Y_pred = label_binarize(index, classes=[i for i in range(10)])
